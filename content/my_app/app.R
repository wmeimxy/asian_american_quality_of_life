...
AALoQ_full <- read_csv("Final_Report_of_the_Asian_American_Quality_of_Life__AAQoL_ copy.csv", col_types=cols('Survey ID' = col_double(), Age=col_double(), 'Education Completed' = col_double(), 'Household Size' = col_double(), Grandparent = col_double(), 'Other Relative' = col_double(), Other=col_double(), 'Self Employed Full Time' = col_double(), 'Self Employed Part Time' = col_double(), Disabled = col_double(),Unemployed = col_double(),'Other Employement' = col_double(),'Achieving Ends Meet' = col_double(),'Duration of Residency' = col_double(),'Primary Language' = col_double(), Discrimination = col_double(),'Hygiene Assistance' = col_double(),Smoking = col_double(),Drinking = col_double(), 'Regular Exercise' = col_double()))
...
temp <- NAAS_clean %>% filter(!is.na(Education) & !is.na(Party_Voting)) %>% count(Education, Party_Voting)

parties <- temp %>% pull(Party_Voting) %>% unique() %>% sort()



ui <- fluidPage(
  titlePanel("Political Party vs Educational level"),
  sidebarLayout(
    sidebarPanel(
      selectInput(inputId = "parties", label = "Party", choices = parties, multiple = TRUE, selected = "Democrat")
    ),
    mainPanel(plotOutput("distPlot"),
              h6("
The message we want to convey through interactive graphs is to let the viewers better understand the life and opinions of Asian Americans. The interaction graphs give a visual way to explain the relationship between different factors that might influence the Asian American community. "),
              h6("In the first interactive draft we created, we would like to make a grid layout where one would select for a political party, graphing educational level vs. count. This would be an interesting way for the user to explore the relationship between their political affiliation and education level by choosing the group they want to view. We predict that the higher the education level, the more left-leaning the political affiliation."), 
              h6("During the process of making interactive graphs, since the vast majority of our data is categorical, we predict having difficulties in plotting y-axes other than counts of the x variable. Another challenge is that there might be more complicated to analyze a larger dataset with more factors and variables after we combine the two datasets. We have to be careful with the data cleaning process to have nice interactive graphs as a result."))
  )
)

server <- function(input, output) {
  output$distPlot <- renderPlot({
    NAAS_clean %>% filter(Party_Voting == input$parties) %>% ggplot(aes_string(x = "Education", fill = "Gender")) + geom_bar()
  })
}
